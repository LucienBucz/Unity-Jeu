...
public class ScriptHabitations : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Debug.Log(nv.LaMaison);
        //nv.LAtelier.LvlBatiment = 1;
        nv.LaMaison.PrixUp();
        // nv.BankMp = 3;
        Debug.Log(nv.LaMaison.PrixAmilioration);
    }
    ...
    private void OnMouseUp()
    {
        if (!baseUi.activeSelf)
        {
            ...
            if (nv.LaMaison.LvlBatiment > 0)
            {
                if (!textAme.activeSelf)
                    textAme.SetActive(true);

                if (!greenUpgradeButton.activeSelf && nv.EstAmeliorableMaison())
                    greenUpgradeButton.SetActive(true);

                else if (!greyUpgradeButton.activeSelf && !nv.EstAmeliorableMaison())
